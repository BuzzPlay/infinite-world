...
export function ProviderSettingsPage({
  ...
  const [defaultStylePreset, setDefaultStylePreset] = useState<StylePreset>(
    settings.defaultStylePreset,
  );
      ...
      <aside className="hidden w-52 shrink-0 border-r border-border/70 p-4 sm:block">
        <nav aria-label="Settings sections">
          <Button
            type="button"
            variant="ghost"
            className="w-full justify-start gap-2 bg-accent font-medium text-accent-foreground hover:bg-accent hover:text-accent-foreground"
          >
        ...
        <div className="w-full max-w-4xl p-5 pb-12 sm:p-10 lg:p-14">
          ...
          <form className="grid gap-4" onSubmit={(event) => void submit(event)}>
            <SectionCard
              ...
              description="Credentials and defaults for new projects"
              flush
            >
              <SettingsRowGroup className="rounded-none border-x-0 border-b-0">
                <SettingsRow
                  label="FAL API key"
                  description={
                    settings.falApiKeyConfigured
                      ? 'Saved in the local service. Leave blank to keep it.'
                      : 'Used for hosted generation.'
                  }
                  htmlFor="fal-api-key"
                >
                  <div className="flex w-[min(20rem,52vw)] items-center gap-2">
                    ...
                    <Input
                      ...
                      placeholder={settings.falApiKeyConfigured ? 'Saved key' : 'Paste your key'}
                      aria-describedby="fal-api-key-status"
                    />
                    {settings.falApiKeyConfigured || falApiKey ? (
                      <Button
                        type="button"
                        size="icon-sm"
                        variant="ghost"
                        title="Clear FAL API key"
                        aria-label="Clear FAL API key"
                        onClick={() => {
                          setFalApiKey('');
                          setClearFalApiKey(true);
                        }}
                      >
                ...
                <SettingsRow label="Connection" description="Current service status">
                  <StatusBadge
                    id="fal-api-key-status"
                    tone={settings.falApiKeyConfigured ? 'success' : 'neutral'}
                  >
                    {settings.falApiKeyConfigured ? (
                      <>
                ...
                <SettingsRow
                  ...
                  description={
                    settings.openaiApiKeyConfigured
                      ? 'Saved in the local service. Leave blank to keep it.'
                      : 'Optional direct prompt provider key.'
                  }
                ...
                >
                  <div className="flex w-[min(20rem,52vw)] items-center gap-2">
                    <Input
                      ...
                      placeholder={settings.openaiApiKeyConfigured ? 'Saved key' : 'Paste your key'}
                    />
                    {settings.openaiApiKeyConfigured || openaiApiKey ? (
                      <Button
                        type="button"
                        size="icon-sm"
                        variant="ghost"
                ...
                <SettingsRow
                  ...
                  description={
                    settings.groqApiKeyConfigured
                      ? 'Saved in the local service. Leave blank to keep it.'
                      : 'Optional fast prompt provider key.'
                  }
                ...
                >
                  <div className="flex w-[min(20rem,52vw)] items-center gap-2">
                    <Input
                      ...
                      placeholder={settings.groqApiKeyConfigured ? 'Saved key' : 'Paste your key'}
                    />
                    {settings.groqApiKeyConfigured || groqApiKey ? (
                      <Button
                        type="button"
                        size="icon-sm"
                        variant="ghost"
                        title="Clear Groq API key"
                        aria-label="Clear Groq API key"
                        onClick={() => {
                          setGroqApiKey('');
                          setClearGroqApiKey(true);
                        }}
                      >
                        <Trash2 className="size-3.5" aria-hidden="true" />
                      </Button>
                ...
                <SettingsRow
                  label="Default model"
                  description="Selected when creating a project"
                ...
                <SettingsRow
                  label="Text model"
                  description="Prompt generation model"
                  htmlFor="llm-text-model"
                >
                  <Input
                    id="llm-text-model"
                    className="w-[min(20rem,52vw)]"
                    value={llmTextModel}
                    onChange={(event) => setLlmTextModel(event.target.value)}
                    placeholder="google/gemini-2.5-flash"
                  />
                </SettingsRow>
                <SettingsRow
                  label="Vision model"
                  description="Model used when a frame is available"
                  htmlFor="llm-vision-model"
                >
                  <Input
                    id="llm-vision-model"
                    className="w-[min(20rem,52vw)]"
                    value={llmVisionModel}
                    onChange={(event) => setLlmVisionModel(event.target.value)}
                    placeholder="google/gemini-2.5-flash"
                  />
                </SettingsRow>
                <SettingsRow
                  label="LLM temperature"
                  description="Prompt variation"
                ...
                <SettingsRow
                  label="Default style"
                  description="Preset for new projects"
                  htmlFor="default-style"
                >
                  <Select
                    value={defaultStylePreset}
                    onValueChange={(value) => setDefaultStylePreset(value as StylePreset)}
                  >
                    <SelectTrigger
                      id="default-style"
                      className="w-44"
                ...
                <SettingsRow
                  ...
                  description={
                    settings.twitchOauthTokenConfigured
                      ? 'Messages can influence the next scene.'
                      : 'Optional Twitch channel input.'
                  }
                ...
                <SettingsRow
                  label="Chat username"
                  description="Only needed with an authenticated chat token."
                ...
                <SettingsRow
                  label="Chat messages"
                  description="Messages considered for the next scene"
                ...
                >
                  <Input
                    ...
                    step="1"
                    value={chatLookback}
                    onChange={(event) => setChatLookback(Number(event.target.value))}
                  />
                </SettingsRow>
                <SettingsRow
                  label="Twitch stream key"
                  description={
                    settings.twitchStreamKeyConfigured
                      ? 'Saved locally. Leave blank to keep it.'
                      : 'Used when Live starts Twitch output without an explicit key.'
                  }
                ...
                >
                  <div className="flex w-[min(20rem,52vw)] items-center gap-2">
                    <Input
                      ...
                      onChange={(event) => {
                        setTwitchStreamKey(event.target.value);
                        setClearTwitchStreamKey(false);
                      }}
                      placeholder={
                        settings.twitchStreamKeyConfigured ? 'Saved key' : 'Paste stream key'
                      }
                    />
                    {settings.twitchStreamKeyConfigured || twitchStreamKey ? (
                      <Button
                        type="button"
                        size="icon-sm"
                        variant="ghost"
                        title="Clear Twitch stream key"
                        aria-label="Clear Twitch stream key"
                        onClick={() => {
                          setTwitchStreamKey('');
                ...
                <SettingsRow
                  ...
                  description={
                    settings.twitchOauthTokenConfigured
                      ? 'Saved locally. Leave blank to keep it.'
                      : 'Optional OAuth token for the chat listener.'
                  }
                ...
                >
                  <div className="flex w-[min(20rem,52vw)] items-center gap-2">
                    <Input
                      ...
                      onChange={(event) => {
                        setTwitchOauthToken(event.target.value);
                        setClearTwitchOauthToken(false);
                      }}
                      placeholder={
                        settings.twitchOauthTokenConfigured ? 'Saved token' : 'oauth token'
                      }
                    />
                    {settings.twitchOauthTokenConfigured || twitchOauthToken ? (
                      <Button
                        type="button"
                        size="icon-sm"
                        variant="ghost"
                        title="Clear Twitch chat token"
                        aria-label="Clear Twitch chat token"
                        onClick={() => {
                          setTwitchOauthToken('');
                ...
                <div className="min-w-0">
                  ...
                  {saved ? (
                    <FieldDescription className="flex items-center gap-1 text-brand-green">
                      <Check className="size-3.5" aria-hidden="true" /> Settings saved
                    </FieldDescription>
                  ) : null}
                </div>
