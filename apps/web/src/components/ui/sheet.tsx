...
export const SheetTrigger = React.forwardRef<
  ...
  Omit<React.ComponentPropsWithoutRef<typeof SheetPrimitive.Trigger>, 'size'> & TriggerVariantProps
...
>(({ className, style, ...props }, ref) => {
  ...
  return (
    <SheetPrimitive.Overlay
      ...
      className={cn(
        'fixed inset-0 bg-black/65 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:animate-in data-[state=open]:fade-in-0',
        className,
      )}
...
const sheetVariants = cva(
  'fixed flex min-h-0 flex-col gap-4 overflow-hidden bg-sidebar p-4 text-foreground shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out h-[calc(100vh-1.4rem)]',
  {
    variants: {
      side: {
        ...
        right:
          'inset-y-0 right-0 h-full w-full border-l border-border/40 data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-sm md:m-[0.7rem] md:h-[calc(100vh-1.4rem)] md:w-3/4 md:rounded-lg',
      },
    },
