...
export type SwitchProps = Omit<React.ComponentProps<typeof SwitchPrimitive.Root>, 'asChild' | 'checked' | 'defaultChecked' | 'onCheckedChange'> & {
  checked?: boolean;
...
export const Switch = React.forwardRef<HTMLButtonElement, SwitchProps>(
  ({
    ...
    const setChecked = React.useCallback((next: boolean) => {
    ...
    const handlePointerDown = React.useCallback((event: React.PointerEvent<HTMLButtonElement>) => {
      ...
      if (event.defaultPrevented || disabled || (event.pointerType === 'mouse' && event.button !== 0)) return;
      setPressed(true);
    ...
    const control = (
      <SwitchPrimitive.Root
        ...
        disabled={disabled}
        tabIndex={0}
        className={cn('relative shrink-0 cursor-pointer touch-none rounded-full outline-none transition-colors duration-100 focus-visible:ring-1 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background', !label && className)}
        style={{ width: TRACK_WIDTH, height: TRACK_HEIGHT, backgroundColor: isChecked ? 'var(--brand-blue)' : 'var(--accent)' }}
        onPointerEnter={(event) => { onPointerEnterProp?.(event); if (event.pointerType === 'mouse') setHovered(true); }}
        onPointerLeave={(event) => { onPointerLeaveProp?.(event); setHovered(false); }}
        onPointerDown={handlePointerDown}
        onPointerMove={handlePointerMove}
        onPointerUp={handlePointerUp}
        onPointerCancel={handlePointerCancel}
        onCheckedChange={(next) => { if (!didDrag.current) setChecked(next); }}
        {...props}
      >
        <SwitchPrimitive.Thumb asChild>
          <m.span className="absolute top-0 left-0 block rounded-full bg-white shadow-sm" initial={false} style={{ x: motionX }} animate={{ y: thumbY, width: thumbWidth, height: thumbHeight }} transition={hasMounted.current ? spring.moderate : { duration: 0 }} />
        </SwitchPrimitive.Thumb>
      </SwitchPrimitive.Root>
    );

    return label ? <label htmlFor={controlId} className={cn('relative z-10 flex cursor-pointer items-center gap-2.5 px-3 py-2 select-none', disabled && 'pointer-events-none opacity-50', className)}>{control}<span className={cn('text-sm transition-colors duration-100', isChecked ? 'text-foreground' : 'text-muted-foreground')}>{label}</span></label> : control;
  },
);
