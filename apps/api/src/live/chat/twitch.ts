...
export class TwitchChat {
  ...
  async connect(settings: ProviderState) {
    ...
    const identity = settings.twitchUsername && settings.twitchOauthToken
    ...
    client.on('message', (_channel: string, tags: Record<string, string | undefined>, message: string, self: boolean) => {
      if (self) return;
      this.messages.push({ username: tags['display-name'] ?? tags.username ?? 'viewer', message, receivedAt: new Date().toISOString() });
