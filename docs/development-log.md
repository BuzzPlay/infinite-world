...
- Created the `apps/web`, `apps/api`, `packages/api-contract`, `runners`, and `docs` boundaries.
...
- Added demo scene generation so the local workflow runs without provider credentials.
